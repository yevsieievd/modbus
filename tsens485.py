...
class Tsens485(abstract_driver.AbstractDriver):
    '''
    classdocs
    '''
...
#        self.cursor = cursor
        ...
        '''
    def get_status(self):
        ctrl_state=self.multiple_read(0, 8)
        if ctrl_state:
    ...
    def synchonise(self):
        ...
        if ctrl_state:
            ...
            if param_res:
                if ctrl_state[0] < 32767:
                    ctrl_state[0] = (float(ctrl_state[0])/100)
                else:
                    ctrl_state[0] = (float(ctrl_state[0] - 65536)/100)

                ctrl_state[1] = (float(ctrl_state[1])/100)
                ctrl_state[2] = (float(ctrl_state[2])/100)
                print ctrl_state
...
if __name__ == '__main__':
    ...
    cursor = db.cursor()
    tsens = Tsens485('/dev/ttyS0', 1, 3, cursor)
    state = tsens.get_status()
    if state:
        print "Tsens returne = %s" % ' '.join(str(i) for i in tsens.get_status())
